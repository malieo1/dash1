...
enum ProjectsType {
  EMPLOYE = 'Nombre des employés',
  STRUCTURE = 'Nombre des structures',
  GROUP = 'Nombre des groupes'
}
...
export class ProjectStatChartComponent implements OnInit {
  ...
  public changeDateType(dateType: string, projectType: string): void {
    switch (projectType) {
      case this.projectsType.EMPLOYE:
        switch (dateType) {
          case 'Weekly':
      ...
      break;
      case this.projectsType.STRUCTURE:
        switch (dateType) {
          case 'Weekly':
